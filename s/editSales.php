...
$sales = $db->query("SELECT * FROM sales WHERE id=$id")->fetchAll(2)[0];
...
if (!empty($_POST)) {
    ...
    if ($db->query("UPDATE sales SET name='$name', image='$image', text='$text', hidden='$hidden' WHERE id='$id'")) {
        echo "<script>
        ...
        <div class="main__product df">
            <div class="addP df jcc">
                <form action="#" method="POST" class="df ai">
                    ...
                    <input type="name" name="name" id="name" placeholder="Код" value="<?= $sales["name"] ?>">
                    <label for="image">Изображение</label>
                    <input type="text" name="image" id="image" placeholder="Изображение" value="<?= $sales["image"] ?>">
                    <label for="text">Описание</label>
                    <input type="text" name="text" id="text" placeholder="Описание" value="<?= $sales["text"] ?>">
                    <fieldset id="hidden" class=".fieldset df ai jcc">
                        <legend class="df jcc">Скрыт?</legend>
                        <input type="radio" id="yes" name="hidden" value="yes" <?php echo ($sales['hidden'] == 'yes') ?  "checked" : "";  ?>>
                        <label for="yes">Да</label>
                        <input type="radio" id="no" name="hidden" value="no" <?php echo ($sales['hidden'] == 'no') ?  "checked" : "";  ?>>
